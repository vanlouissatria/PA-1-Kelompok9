{{-- resources/views/admin/geosite/umkm/index.blade.php --}}
@extends('layouts.admin')

@section('title')
    Kelola UMKM - {{ $geositeTitle }}
@endsection
...
<div class="page-actions">
    <div>
        <h1 class="page-header-title">Data UMKM {{ ucfirst($geosite) }}</h1>
    </div>
    <a href="{{ url('/admin/geosite/'.$geosite.'/umkm/create') }}" class="btn-bi-tambah">
        ...
        <table class="table align-middle mb-0">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    {{-- Nama Usaha --}}
                    <td>
                        <div style="font-weight: 700; color: var(--text-heading);">{{ $item->nama_usaha }}</div>
                    </td>
                    ...
                    {{-- Aksi Sejajar Mendatar Persis Galeri & Penginapan --}}
                    <td style="white-space: nowrap; width: 1%;">
                        <div class="actions-group">
                            {{-- Toggle Status (Ikon Mata) --}}
                            <button type="button"
                            ...
                            {{-- Edit --}}
                            <a href="{{ url('/admin/geosite/'.$geosite.'/umkm/'.$item->id.'/edit') }}" class="action-btn btn-edit" title="Edit" style="margin: 0;">
                                <i class="fas fa-pen"></i>
                            </a>
                            
                            {{-- Hapus --}}
                            <form action="{{ url('/admin/geosite/'.$geosite.'/umkm/'.$item->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus UMKM ini?');" class="d-inline-block" style="margin: 0; padding: 0;">
                                @csrf
                                @method('DELETE')
                ...
                <tr>
                    <td colspan="8" class="text-center py-5 text-secondary">
                        Data UMKM masih kosong. <a href="{{ url('/admin/geosite/'.$geosite.'/umkm/create') }}" class="text-decoration-none" style="color: var(--bi-blue);">Tambah sekarang</a>
                    </td>
                </tr>
                @endforelse
            </tbody>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    toggleButtons.forEach(button => {
        button.addEventListener('click', function() {
            ...
            fetch(`{{ url('/admin/geosite/'.$geosite.'/umkm/toggle-status') }}/${itemId}`, {
                method: 'POST',
                headers: {
