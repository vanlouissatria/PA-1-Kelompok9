...
<div class="card card-bi">
    <div class="card-header card-header-bi">
        <h5><i class="fas fa-tools me-2" style="color: #D4AF37;"></i> Edit Fasilitas: {{ $fasilitas->nama }}</h5>
    </div>
    ...
    <div class="card-body p-4">
        <form action="{{ url('/admin/geosite/'.$geosite.'/fasilitas/'.$fasilitas->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="nama" class="form-control form-control-bi" value="{{ old('nama', $fasilitas->nama) }}" required>
            </div>
            
            <div class="mb-4">
                <label class="form-label-bi">Deskripsi</label>
                <textarea name="deskripsi" class="form-control form-control-bi" rows="3">{{ old('deskripsi', $fasilitas->deskripsi) }}</textarea>
            </div>
            
            <div class="mb-4">
                <label class="form-label-bi">Harga</label>
                <input type="number" name="harga" class="form-control form-control-bi" value="{{ old('harga', $fasilitas->harga) }}">
            </div>
