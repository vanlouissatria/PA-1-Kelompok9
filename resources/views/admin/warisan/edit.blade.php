...
<div class="p-6 max-w-2xl">
    <h1 class="text-2xl font-bold text-gray-800 mb-6">Edit Item Warisan Alam dan Budaya</h1>
    ...
    <form method="POST" action="{{ route('admin.warisan.update', $warisan->id) }}"
        ...
        <div>
            <label class="block font-medium text-gray-700 mb-1">Gambar Baru
                <span class="text-gray-400 text-sm">(kosongkan jika tidak diganti)</span>
            </label>
            @if($warisan->gambar)
                <img src="{{ $warisan->gambar }}" alt="{{ $warisan->judul }}"
                     class="h-32 rounded-lg mb-2 object-cover">
            @endif
        ...
        <div class="flex items-center gap-2">
            <input type="checkbox" name="status" id="status"
                   {{ $warisan->status ? 'checked' : '' }} class="w-4 h-4">
