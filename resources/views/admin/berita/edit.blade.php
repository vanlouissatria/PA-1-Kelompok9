...
@section('content')
<style>
    /* Tema Elegan (Blue & Gold) - Disamakan dengan Galeri & Destinasi */
    :root {
    ...
    /* Style tombol toggle status mata publikasi */
    .btn-toggle-status {
            ...
            <div class="row">
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <input type="text" name="penulis" class="form-control @error('penulis') is-invalid @enderror" 
                           value="{{ old('penulis', $berita->penulis) }}" placeholder="Masukkan nama penulis (opsional)">
                    @error('penulis')
                        <div class="invalid-feedback text-danger">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="col-md-12 mb-3">
                    ...
                    <textarea name="konten" class="form-control @error('konten') is-invalid @enderror" 
                              rows="10" required placeholder="Tuliskan isi konten berita secara lengkap">{{ old('konten', $berita->konten) }}</textarea>
                    @error('konten')
                        <div class="invalid-feedback text-danger">{{ $message }}</div>
                    @enderror
                </div>
                ...
                {{-- Status Publikasi --}}
                <div class="col-md-6 mb-3">
                    <label class="form-label">Status Tampilkan Berita</label>
                    <input type="hidden" name="status" value="0">
                    <div class="form-check p-0 m-0">
                        <input class="form-check-input d-none" type="checkbox" name="status" id="statusSwitch" value="1" {{ $berita->status ? 'checked' : '' }}>
                        <label class="btn-toggle-status" for="statusSwitch">
                            <i class="fas {{ $berita->status ? 'fa-eye' : 'fa-eye-slash' }}" id="statusIcon" style="font-size: 1.1rem;"></i>
                        </label>
                    </div>
                </div>
            </div>
...
<script>
    // Live update preview gambar saat memilih file baru di komputer
